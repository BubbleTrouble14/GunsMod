...
public class ClientEventHandler
{
	...
	private static final ResourceLocation SCOPE_OVERLAY =
			new ResourceLocation(Main.MODID, "textures/gui/scope.png");
	...
	public static void init()
	{
		...
		attachment =
				new KeyBinding("key.attachment", Keyboard.KEY_M, Main.MODID);
	...
	public static void handleClick(MouseEvent evt, EntityPlayer p)
	{
		...
		else if (!rightHandStack.isEmpty())
		{
			if (rightHandStack.getItem() instanceof ItemRangedWeapon)
			{
				rightGun(evt, rightHandStack, world, p);
	...
	public static void leftGun(MouseEvent evt, ItemStack stack, World world,
			EntityPlayer player)
	{
		if (evt.getButton() == 0)
		{
	...
	public static void rightGun(MouseEvent evt, ItemStack stack, World world,
			EntityPlayer player)
	{
		if (evt.getButton() == 1)
		{
			...
			if (att != null && att.isScopePresent())
			{
				...
				else if (mc.objectMouseOver.typeOfHit == Type.BLOCK)
				{
					...
					{
						showScopeOverlap = evt.isButtonstate() && !(Minecraft
	...
	public static void dualGuns(MouseEvent evt, ItemStack rightStack,
			ItemStack leftStack, World world, EntityPlayer player)
	{
		if (onItemleftClick(evt))
		{
			evt.setCanceled(true);
	...
	public static void onFOVUpdate(FOVUpdateEvent evt)
	{
		if (mc.gameSettings.thirdPersonView == 0
				&& (showScopeOverlap && mc.currentScreen == null))
	...
	public void holding(RenderLivingEvent.Pre<EntityLivingBase> event)
	{
		...
		if (!event.isCanceled() && event.getEntity().equals(thePlayer)
				&& stack != null)
		{
	...
	public static void RenderScope()
	{
		...
		vertexbuffer.pos(0.0D, (double) height, -90.0D).tex(0.0D, 1.0D)
				.endVertex();
		vertexbuffer.pos((double) width, (double) height, -90.0D)
