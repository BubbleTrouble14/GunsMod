...
public abstract class ItemRangedWeapon extends ItemBow implements IUpdateAttachments {
	...
	/**
	 * name, durability, maxAmmo, defaultAmmoType, ammoConsumption, shotInterval,
	 * speed, inaccuracy, damage, range, recoil, recoilSneaking, shouldRecoil,
	 * twoHanded
	 */
...
//		});
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int itemSlot, boolean isSelected) {
		...
		if (entity instanceof EntityPlayer) {
			...
			if (!p.getHeldItemOffhand().isEmpty() && p.getHeldItemOffhand().getItem() instanceof ItemRangedWeapon)
				leftStack = p.getHeldItemOffhand();
			if (!p.getHeldItemMainhand().isEmpty() && p.getHeldItemMainhand().getItem() instanceof ItemRangedWeapon)
	...
	private void disableOffHandSlot(EntityPlayer p) {
		if (!p.capabilities.isCreativeMode) {
			...
			if (p.getHeldItemOffhand() != null && p.getHeldItemOffhand().getItem() instanceof ItemRangedWeapon
					...
					&& p.getHeldItemMainhand().getItem() instanceof ItemRangedWeapon) {
	...
	public boolean isReloading(ItemStack stack) {
		checkNBT(stack);
		boolean v = stack.getTagCompound().getBoolean("reloading");
	...
	public boolean canReload(ItemStack stack, EntityPlayer player) {
		if (getAmmoQuantity(stack) == getMaxAmmo())
			return false;
	...
	public int getAmmoQuantityInInventory(ItemStack stack, EntityPlayer player) {
		...
		Item item = Item.REGISTRY.getObject(new ResourceLocation(Main.MODID + ":" + type));// GameRegistry.findItem(Main.MODID,
																							// type);
		int out = 0;
	...
	public void addInformation(ItemStack stack, EntityPlayer playerIn, List<String> tooltip, boolean advanced) {
		...
		if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
			...
			tooltip.add(ChatFormatting.DARK_AQUA + I18n.translate("reload_time.title") + " " + ChatFormatting.WHITE
					+ getReloadDuration() / 20 + I18n.translate("seconds.title"));
			...
			tooltip.add(ChatFormatting.DARK_AQUA + "Duarabilty " + ChatFormatting.WHITE + duarabilty + "/"
					+ stack.getMaxDamage());
