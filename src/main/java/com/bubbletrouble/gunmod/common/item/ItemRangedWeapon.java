		...
		implements IUpdateAttachments
{
	...
	public void initModel()
	{
		ModelResourceLocation scope = new ModelResourceLocation(
				Main.MODID + ":weapons/" + this.getUnlocalizedName() + "_scope",
				"inventory");
		ModelResourceLocation laser = new ModelResourceLocation(
				Main.MODID + ":weapons/" + this.getUnlocalizedName() + "_laser",
				"inventory");
		ModelResourceLocation silencer =
				new ModelResourceLocation(Main.MODID + ":weapons/"
						+ this.getUnlocalizedName() + "_silencer", "inventory");
		ModelResourceLocation flashlight = new ModelResourceLocation(Main.MODID
				+ ":weapons/" + this.getUnlocalizedName() + "_flashlight",
				"inventory");
		ModelResourceLocation reload = new ModelResourceLocation(Main.MODID
				+ ":weapons/" + this.getUnlocalizedName() + "_flashlight",
				"inventory");
		ModelResourceLocation normal = new ModelResourceLocation(
				Main.MODID + ":weapons/" + this.getUnlocalizedName(),
				"inventory");

		ModelBakery.registerItemVariants(this, scope, laser, silencer,
				flashlight, reload, normal);
			...
			int itemSlot, boolean isSelected)
	{
		...
		if (entity instanceof EntityPlayer)
		{
			...
			if (!p.getHeldItemOffhand().isEmpty() && p.getHeldItemOffhand()
					.getItem() instanceof ItemRangedWeapon)
				leftStack = p.getHeldItemOffhand();
			if (!p.getHeldItemMainhand().isEmpty() && p.getHeldItemMainhand()
					.getItem() instanceof ItemRangedWeapon)
				rightStack = p.getHeldItemMainhand();
	...
	private void updateServer(ItemStack rightStack, ItemStack leftStack,
			World world, EntityPlayer p)
	{
		if (!rightStack.isEmpty())
		{
			...
			{
				rightW.setReloadTicks(rightStack, p,
						rightW.getReloadTicks(rightStack) + 1);
				if (rightW.getReloadTicks(rightStack) >= rightW
						.getReloadDuration())
				{
			...
			{
				rightW.setRecoilTicks(rightStack, p,
						rightW.getRecoilTicks(rightStack) + 1);

				if (rightW.getRecoilTicks(rightStack) >= rightW.recoilDelay())
				{
					rightW.recoilDown(p, rightW.getRecoil(),
							rightW.getRecoilSneaking(),
							rightW.getShouldRecoil());
					Main.modChannel.sendTo(new RecoilRightGun(),
							(EntityPlayerMP) p);
		...
		if (!leftStack.isEmpty())
		{
			...
			{
				leftW.setReloadTicks(leftStack, p,
						leftW.getReloadTicks(leftStack) + 1);

				if (leftW.getReloadTicks(leftStack) >= leftW
						.getReloadDuration())
				{
			...
			{
				leftW.setRecoilTicks(leftStack, p,
						leftW.getRecoilTicks(leftStack) + 1);

				if (leftW.getRecoilTicks(leftStack) >= leftW.recoilDelay())
				{
					leftW.recoilDown(p, leftW.getRecoil(),
							leftW.getRecoilSneaking(), leftW.getShouldRecoil());
					Main.modChannel.sendTo(new RecoilLeftGun(),
							(EntityPlayerMP) p);
	...
	private void updateClient(ItemStack rightStack, ItemStack leftStack,
			World world, EntityPlayer p)
	{
	...
	private void disableOffHandSlot(EntityPlayer p)
	{
		...
		{
			if (p.getHeldItemOffhand() != null && p.getHeldItemOffhand()
					.getItem() instanceof ItemRangedWeapon)
			{
				ItemRangedWeapon w =
						(ItemRangedWeapon) p.getHeldItemOffhand().getItem();
				if (w.IsTwoHanded())
				{
			...
			if (p.getHeldItemOffhand() != null
					&& p.getHeldItemOffhand()
							.getItem() instanceof ItemRangedWeapon
					&& p.getHeldItemMainhand() != null
					&& p.getHeldItemMainhand()
							.getItem() instanceof ItemRangedWeapon)
			{
				ItemRangedWeapon rw =
						(ItemRangedWeapon) p.getHeldItemMainhand().getItem();
	...
	public void setReloadTicks(ItemStack stack, EntityPlayer player,
			int reloadTicks)
	{
	...
	public void setRecoilTicks(ItemStack stack, EntityPlayer player,
			int recoilTicks)
	{
	...
	public void setReloading(ItemStack stack, EntityPlayer player,
			boolean reloading)
	{
	...
	public void setUpdateModel(ItemStack stack, EntityPlayer player,
			boolean model)
	{
	...
	public void recoilDown(EntityPlayer entityIn, float recoil,
			float recoilSneaking, boolean shouldRecoil)
	{
	...
	public void recoilUp(EntityPlayer entityIn, float recoil,
			float recoilSneaking, boolean shouldRecoil)
	{
		if (shouldRecoil) entityIn.rotationPitch -=
				entityIn.isSneaking() ? recoil : recoilSneaking;
	}
	...
	public void hasAmmoAndConsume(ItemStack stack, EntityPlayer player)
	{
		...
		for (int i = 0; i < inventory.size(); i++)
		{
			...
			if (invStack != null) if (isValidProjectile(invStack.getItem()))
			{
				...
				int ammo = stackSize < this.getMaxAmmo() - ammoFinal ? stackSize
						: this.getMaxAmmo() - ammoFinal;
				ammoFinal += ammo;

				// Set stack size
				invStack.setCount(stackSize - ammo);
				if (invStack.getCount() < 1) inventory.set(i, ItemStack.EMPTY);
	...
	public boolean canReload(ItemStack stack, EntityPlayer player)
	{
		...
		if (getMaxAmmo() == 1) { return !player.capabilities.isCreativeMode; }
		return getAmmoQuantity(stack) < getMaxAmmo()
	...
	public ItemProjectile findAvailableAmmo(EntityPlayer player)
	{
		for (ItemProjectile projectile : projectiles)
		{
			for (int i = 0; i < player.inventory.getSizeInventory(); i++)
			{
				...
				if (this.isProjectile(stack)) { return projectile; }
			}
		}
		return null;
	}
	...
	public int getAmmoQuantityInInventory(ItemStack stack, EntityPlayer player)
	{
		...
		Item item = Item.REGISTRY
				.getObject(new ResourceLocation(Main.MODID + ":" + type));// GameRegistry.findItem(Main.MODID, type);
		int out = 0;
		if (type != null)
		{ // && inventory.hasItemStack(item)
	...
	public void soundEmpty(ItemStack itemstack, World world,
			EntityPlayer entityplayer)
	{
		world.playSound(entityplayer, entityplayer.getPosition(),
				SoundEvents.UI_BUTTON_CLICK, SoundCategory.PLAYERS, 1.0F,
				1.0F / 0.8F);
	}
	...
	public void soundCharge(ItemStack stack, World world, EntityPlayer player)
	{
		...
		world.playSound(player, pos,
				SoundEvent.REGISTRY.getObject(new ResourceLocation(Main.MODID
						+ ":" + this.getUnlocalizedName() + "_reload")),
				SoundCategory.PLAYERS, 0.7F,
				0.9F / (getItemRand().nextFloat() * 0.2F + 0.0F));
	}

	public abstract int getReloadDuration();

	public void applyProjectileEnchantments(EntityProjectile entity,
			ItemStack itemstack)
	{
		int damage = EnchantmentHelper.getEnchantmentLevel(
				Enchantment.getEnchantmentByID(49), itemstack);
		if (damage > 0)
		{
			entity.setDamage(damage);
		}

		int knockback = EnchantmentHelper.getEnchantmentLevel(
				Enchantment.getEnchantmentByID(48), itemstack);
		if (knockback > 0)
		{
		...
		if (EnchantmentHelper.getEnchantmentLevel(
				Enchantment.getEnchantmentByID(50), itemstack) > 0)
		{
			entity.setFire(100);
		}
	}

	public final void postShootingEffects(ItemStack itemstack,
			EntityPlayer entityplayer, World world)
	{
		effectPlayer(itemstack, entityplayer, world);
		effectShoot(entityplayer, itemstack, world, entityplayer.posX,
				entityplayer.posY, entityplayer.posZ, entityplayer.rotationYaw,
	...
	public void effectShoot(EntityPlayer p, ItemStack stack, World world,
			double x, double y, double z, float yaw, float pitch)
	{
		String soundPath =
				Main.MODID + ":" + this.getUnlocalizedName() + "_shoot";
		...
		world.playSound(p, p.getPosition(),
				SoundEvent.REGISTRY.getObject(new ResourceLocation(soundPath)),
				SoundCategory.PLAYERS, 0.7F,
				0.9F / (getItemRand().nextFloat() * 0.2F + 0.0F));

		float particleX = -MathHelper.sin(((yaw + 23) / 180F) * 3.141593F)
				* MathHelper.cos((pitch / 180F) * 3.141593F);
		float particleY = -MathHelper.sin((pitch / 180F) * 3.141593F) - 0.1F;
		float particleZ = MathHelper.cos(((yaw + 23) / 180F) * 3.141593F)
				* MathHelper.cos((pitch / 180F) * 3.141593F);

		for (int i = 0; i < 3; i++)
		{
			world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, x + particleX,
					y + particleY, z + particleZ, 0.0D, 0.0D, 0.0D);
		}
		world.spawnParticle(EnumParticleTypes.FLAME, x + particleX,
				y + particleY, z + particleZ, 0.0D, 0.0D, 0.0D);
	}
	...
	public void afterFire(ItemStack stack, World world, EntityPlayer player)
	{
		...
		if (stack.getItemDamage() + damage > stack.getMaxDamage())
		{
			...
			Item i = Item.REGISTRY
					.getObject(new ResourceLocation(Main.MODID + ":" + type));
			ItemStack s = new ItemStack(i, ammo);
			player.inventory.addItemStackToInventory(s);
		}
		else if (ammo < 1)
		{
			if (hasAmmoInInventory(player) && player.world.isRemote)
	...
	protected EntityProjectile createProjectile(ItemStack stack, World world,
			EntityPlayer player)
	{
		try
		{
			...
			Class<?> c = Class.forName("com.bubbletrouble.gunmod.common.entity."
					+ ProjectileType.valueOf(type.toUpperCase()).getEntity());
			Constructor<?> con =
					c.getConstructor(World.class, EntityLivingBase.class,
	...
	{
		tooltip.add(ChatFormatting.DARK_AQUA + "Damage " + ChatFormatting.WHITE
				+ this.getDamage() + " Hearts");
		if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT))
		{
			int duarabilty = stack.getMaxDamage() - stack.getItemDamage();
			tooltip.add(ChatFormatting.DARK_AQUA
					+ I18n.translate("reload_time.title") + " "
					+ ChatFormatting.WHITE + getReloadDuration() / 20
					+ I18n.translate("seconds.title"));
			tooltip.add(ChatFormatting.DARK_AQUA + "Range "
					+ ChatFormatting.WHITE + this.getRange() + " blocks");
			tooltip.add(ChatFormatting.DARK_AQUA + "Has recoil : "
					+ ChatFormatting.WHITE + getShouldRecoil());
			tooltip.add(ChatFormatting.DARK_AQUA + "Duarabilty "
					+ ChatFormatting.WHITE + duarabilty + "/"
					+ stack.getMaxDamage());
			tooltip.add(ChatFormatting.DARK_AQUA + "Two-Handed : "
					+ ChatFormatting.WHITE + IsTwoHanded());
		}
	}
			...
			ItemStack newStack, boolean slotChanged)
	{
		...
		{
			if (InventoryAttachment.create(oldStack)
					.getStackInSlot(0) == InventoryAttachment.create(newStack)
							.getStackInSlot(0))
				return true;
